...
impl TransactionPoolServer for TransactionPoolApi {
    async fn send_message(
        ...
        let signed_gearexe_tx = SignedOffchainTransaction::try_from_parts(gearexe_tx, signature)
            .map_err(|e| {
                log::error!("{e}");
                errors::internal()
            })?;

        log::debug!("Called send_message with vars: {signed_gearexe_tx:#?}");

        let (response_sender, response_receiver) = oneshot::channel();
        self.rpc_sender
            .send(RpcEvent::OffchainTransaction {
                transaction: signed_gearexe_tx,
