...
mod tests {
    ...
    fn test_valid_mortality() {
        ...
        let signed_tx = tests::generate_signed_gearexe_tx(block_hash);
    ...
    fn test_invalid_mortality_non_existent_block() {
        ...
        let invalid_transaction = tests::generate_signed_gearexe_tx(non_window_block_hash);
    ...
    fn test_invalid_mortality_rotten_tx() {
        ...
        let transaction1 = TxValidator::new(
            tests::generate_signed_gearexe_tx(first_block_hash),
            db.clone(),
        )
        ...
        let transaction2 = TxValidator::new(
            tests::generate_signed_gearexe_tx(second_block_hash),
            db.clone(),
        )
        .with_mortality_check()
        .validate()
    ...
    fn test_uniqueness_validation() {
        let db = Database::memory();
        let transaction = tests::generate_signed_gearexe_tx(H256::random());
