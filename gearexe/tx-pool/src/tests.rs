...
pub(crate) fn generate_signed_gearexe_tx(reference_block_hash: H256) -> SignedOffchainTransaction {
...
async fn test_add_transaction() {
    ...
    let signed_gearexe_tx = generate_signed_gearexe_tx(tx_reference_block_hash);
    assert!(tx_pool.validate(signed_gearexe_tx.clone()).is_ok());
    ...
    // Rotten block hash
    let invalid_tx = generate_signed_gearexe_tx(tx_reference_block_hash);
