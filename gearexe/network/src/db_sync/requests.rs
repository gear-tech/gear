...
use gearexe_common::gear::CodeState;
...
gearexe_service_utils::task_local! {
        ...
        for either in diff {
            match either {
                EitherOrBoth::Both(req_key, (resp_key, resp_val)) => {
                    ...
                    if req_key != gearexe_db::hash(&resp_val) {
...
mod tests {
    ...
    fn validate_data_stripped() {
        let hash1 = gearexe_db::hash(b"1");
        let hash2 = gearexe_db::hash(b"2");
        let hash3 = gearexe_db::hash(b"3");
