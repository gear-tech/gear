...
const STREAM_PROTOCOL: StreamProtocol =
    StreamProtocol::new(concat!("/gearexe/db-sync/", env!("CARGO_PKG_VERSION")));
...
pub(crate) mod tests {
    ...
    use gearexe_common::{StateHashWithQueueSize, db::BlockMetaStorageWrite};
    use gearexe_db::MemDb;
    use libp2p::{
        Swarm, Transport,
    ...
    async fn excessive_data_stripped() {
        ...
        let data_0 = gearexe_db::hash(&DATA[0]);
        let data_1 = gearexe_db::hash(&DATA[1]);
        let data_2 = gearexe_db::hash(&DATA[2]);
    ...
    async fn retry() {
        ...
        let request_key = gearexe_db::hash(b"test");
        let request_id = alice
            .behaviour_mut()
