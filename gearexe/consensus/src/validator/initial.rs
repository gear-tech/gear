...
use gearexe_common::{Address, SimpleBlockData, db::OnChainStorageRead};
        ...
        match &self.state {
            State::WaitingForSyncedBlock(block) if block.hash == block_hash => {
                ...
                if my_address == producer {
                    ...
                    // TODO #4636: add test (in gearexe-service) for case where is not validator for current block
...
mod tests {
    ...
    use gearexe_common::db::OnChainStorageWrite;
    use gprimitives::H256;
    use nonempty::nonempty;
