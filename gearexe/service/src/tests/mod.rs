...
use gearexe_db::{Database, verifier::IntegrityVerifier};
use gearexe_observer::EthereumConfig;
use gearexe_prometheus::PrometheusConfig;
use gearexe_rpc::RpcConfig;
use gearexe_runtime_common::state::{Expiring, MailboxMessage, PayloadLookup, Storage};
use gearexe_tx_pool::{OffchainTransaction, RawOffchainTransaction, TxPoolEvent};
use gear_core::{
    ids::prelude::*,
...
async fn basics() {
    ...
    config.network = Some(gearexe_network::NetworkConfig::new_local(
        tmp_dir.join("net"),
    ));
...
async fn mailbox() {
    ...
    let expiry = block_data.header.height - 1 + gearexe_runtime_common::state::MAILBOX_VALIDITY;
...
async fn multiple_validators() {
    ...
    log::info!("📗 Stop validator 0 and check, that gearexe is still working");
    ...
    log::info!("📗 Stop validator 1 and check, that gearexe is not working after");
    ...
    log::info!(
        "📗 Re-start validator 0 and check, that now gearexe is working, validator 1 is still stopped"
    );
...
async fn tx_pool_gossip() {
    ...
    let signed_gearexe_tx = {
        ...
        env.signer.signed_data(sender_pub_key, gearexe_tx).unwrap()
    };

    let (transaction, signature) = signed_gearexe_tx.clone().into_parts();
    ...
    let tx_hash = signed_gearexe_tx.tx_hash();
    let node1_db_tx = node1
        .db
        .get_offchain_transaction(tx_hash)
        .expect("tx not found");
    assert_eq!(node1_db_tx, signed_gearexe_tx);
}
