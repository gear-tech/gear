...
use gearexe_consensus::{ConsensusService, SimpleConnectService, ValidatorService};
use gearexe_db::Database;
use gearexe_ethereum::{Ethereum, deploy::EthereumDeployer, router::RouterQuery};
use gearexe_network::{NetworkConfig, NetworkService, export::Multiaddr};
use gearexe_observer::{EthereumConfig, ObserverEvent, ObserverService};
use gearexe_processor::Processor;
use gearexe_rpc::{RpcConfig, RpcService, test_utils::RpcClient};
use gearexe_signer::Signer;
use gearexe_tx_pool::TxPoolService;
...
impl TestEnv {
    ...
    pub async fn next_block_producer_index(&self) -> usize {
        ...
        gearexe_consensus::block_producer_index(
            self.validators.len(),
            ...
            if let Some(config) = self.validator_config.as_ref() {
                Box::pin(
                    ValidatorService::new(
                        self.signer.clone(),
                        self.db.clone(),
                        gearexe_consensus::ValidatorConfig {
