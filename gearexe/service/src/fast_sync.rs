...
use gearexe_compute::ComputeService;
use gearexe_db::{
    Database,
    iterator::{
...
use gearexe_ethereum::mirror::MirrorQuery;
use gearexe_network::{NetworkEvent, NetworkService, db_sync};
use gearexe_observer::ObserverService;
use gearexe_runtime_common::{
    ScheduleRestorer,
    state::{
            ...
            match metadata {
                ...
                RequestMetadata::MemoryPages => {
                    let memory_pages: MemoryPages =
                        Decode::decode(&mut &data[..]).expect("`db-sync` must validate data");
                    gearexe_db::visitor::walk(&mut manager, MemoryPagesNode { memory_pages });
                }
                ...
                RequestMetadata::MessageQueue => {
                    let message_queue: MessageQueue =
                        Decode::decode(&mut &data[..]).expect("`db-sync` must validate data");
                    gearexe_db::visitor::walk(&mut manager, MessageQueueNode { message_queue });
                }
                RequestMetadata::Waitlist => {
                    let waitlist: Waitlist =
                        Decode::decode(&mut &data[..]).expect("`db-sync` must validate data");
                    gearexe_db::visitor::walk(&mut manager, WaitlistNode { waitlist });
                }
                RequestMetadata::Mailbox => {
                    let mailbox: Mailbox =
                        Decode::decode(&mut &data[..]).expect("`db-sync` must validate data");
                    gearexe_db::visitor::walk(&mut manager, MailboxNode { mailbox });
                }
                RequestMetadata::UserMailbox => {
                    let user_mailbox: UserMailbox =
                        Decode::decode(&mut &data[..]).expect("`db-sync` must validate data");
                    gearexe_db::visitor::walk(&mut manager, UserMailboxNode { user_mailbox });
                }
                RequestMetadata::DispatchStash => {
                    let dispatch_stash: DispatchStash =
                        Decode::decode(&mut &data[..]).expect("`db-sync` must validate data");
                    gearexe_db::visitor::walk(&mut manager, DispatchStashNode { dispatch_stash });
                }
