...
use crate::{Error, MAX_POSSIBLE_ALLOCATION, MAX_WASM_PAGES, Memory, PAGE_SIZE};
...
impl FreeingBumpHeapAllocator {
    ...
    fn bump(bumper: &mut u32, size: u32, memory: &mut impl Memory) -> Result<u32, Error> {
        ...
        if required_size > memory.size() {
            let required_pages =
                pages_from_size(required_size).ok_or(Error::AllocatorOutOfSpace)?;
            ...
            debug_assert_eq!(
                memory.pages(),
                next_pages,
                "Number of pages should have increased!"
            );
        }
...
mod tests {
    ...
    impl Memory for MemoryInstance {
        ...
        fn grow(&mut self, pages: u32) -> Result<(), ()> {
            if self.pages() + pages > self.max_wasm_pages {
                ...
                self.data
                    .resize(((self.pages() + pages) * PAGE_SIZE) as usize, 0);
                Ok(())
            }
        }
    }
    ...
    fn should_read_and_write_u64_correctly() {
        ...
        // then
        let value = MemoryExt::read_le_u64(&mem, 40).unwrap();
    ...
    fn poison_oom() {
        ...
        assert_eq!(
            Error::AllocatorOutOfSpace,
            heap.allocate(&mut mem, PAGE_SIZE).unwrap_err()
        );
