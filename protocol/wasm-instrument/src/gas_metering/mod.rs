...
fn insert_metering_calls(
    ...
    let new_instrs_len = instructions.len() + 2 * block_cost_instrs;
    let original_instrs = mem::replace(instructions, Vec::with_capacity(new_instrs_len));
    let new_instrs = instructions;
