...
impl ExtManager {
    ...
    pub(crate) fn read_state_bytes_using_wasm<E: Encode + TypeInfo + 'static>(
        ...
        // Call fn metadata() that's generated by `metawasm` macro and get function input type.
        let metadata =
            core_processor::informational::execute_for_reply::<WasmiEnvironment<Ext, _>, _>(
                "metadata".to_string(),
                mapping_code.clone(),
        ...
        let mut mapping_code_payload = if let Some(args) = args {
            // Create a new registry to store the provided input type (`E`) and compare it with one implemented in wasm.
