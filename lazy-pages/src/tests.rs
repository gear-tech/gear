...
fn test_mprotect_pages() {
    ...
    unsafe {
        for p in pages_protected.iter().flat_map(Interval::from) {
            let addr = page_begin + offset(p) + 1;
        ...
        for p in pages_unprotected.iter().flat_map(Interval::from) {
            let addr = page_begin + offset(p) + 1;
            let x = *(addr as *mut u8);
