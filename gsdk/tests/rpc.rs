...
async fn test_calculate_handle_gas() -> Result<()> {
    ...
    let salt = vec![];
    let pid = ProgramId::generate_with_nonce(CodeId::generate(demo_messager::WASM_BINARY), &salt, 0u32);
...
async fn test_calculate_reply_gas() -> Result<()> {
    ...
    let salt = vec![];
    let pid = ProgramId::generate_with_nonce(CodeId::generate(demo_waiter::WASM_BINARY), &salt, 0u32);
