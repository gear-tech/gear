...
/// Следующие случаи:
/// 1. Заново инициализировать пытаешься активную или терминэйтед программу
/// 2. ОТправляешь сообщение терминэйтед программе, которая стала такой (изи init или handle (exit), когда
/// сообщение было в очереди. Или когда программа стала терминэйтед, ожидающие сообщения
/// были добавлены в очередь.
/// 
/// TODO сделай подробную проверку того, как в него попасть (когда вызываются set_terminated и т.п.) -> сделай trace 
/// стэйта (т.е., в каком случае стэйта можно сюда попасть, чье и какое сообщение будет процессится в таком случае)
/// и тогда подробнейшим образом поймем, как эта функция вызывается
