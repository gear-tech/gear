...
use gear_core::{
    ...
    message::{
        ContextSettings, DispatchKind, IncomingDispatch, IncomingMessageInfo, Payload,
        PayloadSizeError, ReplyMessage, StoredDispatch,
    },
    ...
    <Ext as Externalities>::UnrecoverableError: BackendSyscallError,
{
    ...
    let context_presented = dispatch.context().is_some();
    let message_info = dispatch.info();
    ...
    match exec_result {
        Ok(res) => {
            ...
            Ok(match res.kind {
                DispatchResultKind::Trap(reason) => process_execution_error_with_dispatch(
                    res.dispatch,
                    program_id,
...
fn process_error_with_dispatch(
...
fn process_error(
    ...
    message_info: IncomingMessageInfo,
    program_id: ProgramId,
    gas_burned: u64,
    system_reservation_ctx: SystemReservationContext,
    case: ProcessErrorCase,
    ...
    let origin = message_info.source();
    let value = message_info.value();
    ...
    if let ProcessErrorCase::ExecutionFailed(reason) = &case {
        ...
        if system_reservation_ctx.has_any()
            && !message_info.is_error_reply()
            && !matches!(kind, DispatchKind::Signal | DispatchKind::Init)
        {
    ...
    if !message_info.is_reply() && kind != DispatchKind::Signal {
        ...
        let dispatch = ReplyMessage::system(message_info.id(), err_payload, err).into_dispatch(
            program_id,
            message_info.source(),
            message_info.id(),
        );
...
/// Helper function for journal creation in trap/error case using dispatch.
pub fn process_execution_error_with_dispatch(
    dispatch: IncomingDispatch,
    program_id: ProgramId,
    gas_burned: u64,
    system_reservation_ctx: SystemReservationContext,
