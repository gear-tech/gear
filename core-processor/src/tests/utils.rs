...
//! Test utilities for core-processor tests.
...
/// A single step in a test scenario with an expectation about success/failure.
...
impl ExecutionStep {
    pub fn success(label: &'static str, kind: DispatchKind, payload: Payload) -> Self {
        Self {
            label,
            kind,
            payload,
    ...
    pub fn failure(label: &'static str, kind: DispatchKind, payload: Payload) -> Self {
        Self {
            label,
            kind,
            payload,
...
/// Empty lazy pages storage for tests.
#[derive(Debug)]
...
impl<'a> MemoryView<'a> {
    pub fn new(info: &'a ExtInfo) -> Self {
        Self {
            pages: &info.pages_data,
        }
    }

    pub fn read_into(&self, offset: u32, buffer: &mut [u8]) {
        if buffer.is_empty() {
            return;
        }
    ...
    pub fn read_array<const N: usize>(&self, offset: u32) -> [u8; N] {
        let mut buf = [0u8; N];
    ...
    pub fn read_u128(&self, offset: u32) -> u128 {
    ...
    pub fn find_sequence(&self, needle: &[u8]) -> Option<u32> {
        if needle.is_empty() {
            return Some(0);
        }
    ...
    pub fn find_all(&self, needle: &[u8]) -> Vec<u32> {
        if needle.is_empty() {
            return vec![];
        }
    ...
    pub fn read_utf8(&self, offset: u32, len: usize) -> String {
...
impl TokenSnapshot {
    pub fn from_ext(info: &ExtInfo) -> Self {
...
/// Program fixture for loading test code.
...
/// Create a step that expects success.
pub fn step_success<T: Encode>(label: &'static str, kind: DispatchKind, value: T) -> ExecutionStep {
    ExecutionStep::success(label, kind, encoded_payload(value))
}

/// Create a step that expects failure.
pub fn step_failure<T: Encode>(label: &'static str, kind: DispatchKind, value: T) -> ExecutionStep {
    ExecutionStep::failure(label, kind, encoded_payload(value))
}

/// Encode a value into a payload.
pub fn encoded_payload<T: Encode>(value: T) -> Payload {
    Payload::try_from(value.encode()).expect("Failed to encode payload")
}

/// Assert that pages were populated during execution.
pub fn assert_pages_populated(info: &ExtInfo) {
    assert!(
...
/// Assert that two execution infos have equal pages.
pub fn assert_pages_equal(expected: &ExtInfo, actual: &ExtInfo) {
    assert_eq!(expected.pages_data, actual.pages_data);
}

/// Assert that two execution infos have different pages.
pub fn assert_pages_different(expected: &ExtInfo, actual: &ExtInfo) {
    assert_ne!(expected.pages_data, actual.pages_data);
}

/// Assert that a TotalSupply reply is present in the execution trace.
pub fn assert_total_supply_reply(info: &ExtInfo) {
    assert!(
...
/// Extract the total supply from the execution info if present.
pub fn total_supply(info: &ExtInfo) -> Option<u128> {
...
/// Create an externality for a test step.
pub fn make_ext(step: &ExecutionStep, actor_id: ActorId, code: &Code) -> Ext {
...
/// Get the test message sender actor ID.
pub fn message_sender() -> ActorId {
    let bytes = [1, 2, 3, 4].repeat(8);
    ActorId::try_from(bytes.as_ref()).unwrap()
}

/// Execute the steps sequentially, applying memory snapshots on failures.
pub fn run_sequence<M: MemorySnapshot>(steps: &[ExecutionStep], snapshot: &mut M) -> ExtInfo {
...
/// Execute the steps sequentially without snapshots (for testing divergence).
pub fn run_sequence_without_snapshots<M: MemorySnapshot>(
    steps: &[ExecutionStep],
    _snapshot: &mut M,
) -> Option<ExtInfo> {
    assert!(!steps.is_empty(), "sequence must contain at least one step");
...
/// Create the initial environment for execution.
pub fn create_environment<'a>(
    step: &ExecutionStep,
...
/// Advance the environment to the next step.
pub fn advance_environment<'a, M: MemorySnapshot>(
...
/// Finalize execution and extract the info.
