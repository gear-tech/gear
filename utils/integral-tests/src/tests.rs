...
use sp_runtime::{MultiAddress, Perbill, Perquintill};
...
use crate::util::{
    ...
    Runtime, RuntimeOrigin, RwLock, Staking, StakingRewards, Treasury, ValidatorAccountId,
...
const INITIAL_TOTAL_TOKEN_SUPPLY: u128 = 10_000_000 * TOKEN;
...
pub fn run_test(params: &TestParams, f: impl FnOnce(&TestParams, &Arc<RwLock<PoolState>>)) {
    ...
    let (mut ext, pool) = builder
        ...
        .non_stakeable(Perquintill::from_perthousand(435))
        .pool_balance(Perquintill::from_percent(14) * params.total_supply)
        .ideal_stake(params.ideal_stake)
...
fn inflation_without_nominators() {
    ...
    for vs in validators_sets {
        ...
        run_test(&params, |p, pool| {
            ...
            let initial_total_stakeable = StakingRewards::total_stakeable_tokens();
            let initial_treasury_balance = Balances::free_balance(Treasury::account_id());
            let initial_rewards_pool_balance = StakingRewards::pool();
            let signer = Lazy::get(&SIGNING_KEY).expect("value initialized; qed");

            assert_eq!(initial_total_issuance, params.total_supply);
            assert_eq!(
                initial_total_stakeable,
                Perquintill::from_perthousand(425) * params.total_supply
            );
            ...
            let (expected_payout_0, expected_remainder) =
                pallet_gear_staking_rewards::compute_total_payout(
                    0,
                    initial_total_stakeable,
                    initial_total_issuance,
                    params.ideal_stake,
                    <Runtime as pallet_gear_staking_rewards::Config>::MinInflation::get(),
                    params.target_inflation,
                    <Runtime as pallet_gear_staking_rewards::Config>::Falloff::get(),
                    <Runtime as pallet_gear_staking_rewards::Config>::MaxROI::get(),
                    time_fraction,
                );
            ...
            let rewards_pool_balance = StakingRewards::pool();
            let total_stakeable = StakingRewards::total_stakeable_tokens();
            ...
            // The total stakeable amount has grown respectively
            assert_eq!(
                total_stakeable,
            ...
            let (expected_payout_1, expected_remainder) =
                pallet_gear_staking_rewards::compute_total_payout(
                    total_staked,
                    initial_total_stakeable,
                    initial_total_issuance,
            ...
            let validators_reward_for_era = Staking::eras_validator_reward(1)
                .expect("ErasValidatorReward storage must exist after era end; qed");
            assert_approx_eq!(validators_reward_for_era, expected_payout_1, 100);
            ...
            let validators_total_balance = validators_total_balance(p.validators.clone());
            assert_approx_eq!(
                validators_total_balance,
                num_validators as u128 * 10_u128 * EXISTENTIAL_DEPOSIT + expected_payout_1,
                10
            );
        });
    }
...
#[test]
#[ignore = "inflation calculation is not up-to-date"]
