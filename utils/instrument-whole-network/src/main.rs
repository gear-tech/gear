...
#[rustfmt::skip]
const TESTNET_EXCLUDED_CODES: &[CodeId] = &[
    // `gr_leave` somehow imported twice
    CodeId::new(hex!("4dd9c141603a668127b98809742cf9f0819d591fe6f44eff63edf2b529a556bd")),
    CodeId::new(hex!("90b021503f01db60d0ba00eac970d5d6845f1a757c667232615b5d6c0ff800cc")),
    // `init` entrypoint has invalid signature
    CodeId::new(hex!("8990159f0730dfed622031af63c453d2bcd5644482cac651796bf229f25d23b6")),
    // `handle` entrypoint has invalid signature
    CodeId::new(hex!("e8378f125ec82bb7f399d81b3481d5d62bb5d65749f47fea6cd65f7a48e9c24c")),
    CodeId::new(hex!("d815332c3980386e58d0d191c5161d33824d8a6356a355ccb3528e6428551ab3")),
    // `init` export directly references `gr_leave` import
    CodeId::new(hex!("ec0cc5d401606415c8ed31bfd347865d19fd277eec7d7bc62c164070eb8c241a")),
    // `gr_error` has been removed
    CodeId::new(hex!("10d92d804fc4d42341d5eb2ca04b59e8534fd196621bd3908e1eda0a54f00ab9")),
    CodeId::new(hex!("7ae2b90c96fd65439cd3c72d0c1de985b42400c5ad376d34d1a4cb070191ed2c")),
    // `delay` argument in `gr_reply` was removed
    CodeId::new(hex!("2477bc4f927a3ae8c3534a824d6c5aec9fa9b0f4747a1f1d4ae5fabbe885b111")),
    CodeId::new(hex!("7daa1b4f3a4891bda3c6b669ca896fa12b83ce4c4e840cf1d88d473a330c35fc")),
    // `gr_pay_program_rent` has been removed
    CodeId::new(hex!("4a0bd89b42de7071a527c13ed52527e941dcda92578585e1139562cdf8a1063e")),
    CodeId::new(hex!("d483a0e542ad20996b38a2efb1f41e8d863cc1659f1ceb89a79065849fadfeb5")),
    // `ext_logging_log_version_1` import somehow occurred
    CodeId::new(hex!("75e61ed8f08379ff9ea7f69d542dceabf5f30bfcdf95db55eb6cab77ab3ddb56")),
    CodeId::new(hex!("164dfe52b1438c7e38d010bc28efc85bd307128859d745e801c9099cbd82bd4f")),
    CodeId::new(hex!("f92585a339751d7ba9da70a0536936cd8659df29bad777db13e1c7b813c1a301")),
    // `ext_misc_print_utf8_version_1` import somehow occurred
    CodeId::new(hex!("c88b00cfd30d1668ebb50283b4785fd945ac36a4783f8eab39dec2819e06a6c9")),
];

#[derive(Debug, Parser)]
struct Args {
    /// Network to fetch codes from
    #[clap(long)]
    network: Network,
...
async fn fetch_and_instrument(
    ...
    latest_block: H256,
    code_id: CodeId,
) -> anyhow::Result<Option<FailedCode>> {
...
async fn main() -> anyhow::Result<()> {
    ...
    let excluded_codes = if let Network::Testnet = network {
        TESTNET_EXCLUDED_CODES
    } else {
        &[]
    };
