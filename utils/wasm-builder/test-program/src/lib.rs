...
mod tests {
    ...
    fn debug_wasm() {
        assert_eq!(
            fs::read("target/wasm32v1-none/debug/test_program.wasm").unwrap(),
            code::WASM_BINARY,
        );
        assert_eq!(
            fs::read("target/wasm32v1-none/debug/test_program.opt.wasm").unwrap(),
            code::WASM_BINARY_OPT,
        );
        assert!(fs::read("target/wasm32v1-none/debug/test_program.meta.wasm").is_err());
    }
    ...
    fn release_wasm() {
        assert_eq!(
            fs::read("target/wasm32v1-none/release/test_program.wasm").unwrap(),
            code::WASM_BINARY,
        );
        assert_eq!(
            fs::read("target/wasm32v1-none/release/test_program.opt.wasm").unwrap(),
            code::WASM_BINARY_OPT,
        );
        assert!(fs::read("target/wasm32v1-none/release/test_program.meta.wasm").is_err());
    }
}
