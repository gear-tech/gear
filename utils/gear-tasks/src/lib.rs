        ...
        pub async fn run(self) {
            ...
            } in self.rx
            {
                ...
                self.thread_pool.spawn_ok(async move {
                    ...
                    // considered unwind-safe because although `RuntimeImpl` structure
                    // generated by `impl_runtime_apis!()` macro uses `RefCell` inside,
                    // it's created on each API call as a new empty instance that not associated with
                    // the client via some dependent primitives like `RefCell`, pointers, etc.
                    //
                    // NOTE: consideration is implementation detail of Substrate
                    let runtime_api = AssertUnwindSafe(runtime_api);
                    // considered unwind-safe because we imply it's just simple copy-type
                    // without logic like `RefCell` in it
                    let block_hash = AssertUnwindSafe(block_hash);

                    let f = move || {
                        let payload = panic::catch_unwind(|| {
                            runtime_api.execute_task(*block_hash, func_ref, payload)
                        })
                        // we believe runtime API can panic because
                        // every runtime call (WASM or native) is wrapped
                        // in `catch_unwind` in Substrate
                        .map_err(|cause| {
                            if let Some(err) = cause.downcast_ref::<String>() {
                                JoinError::Panic(err.clone())
                            } else if let Some(err) = cause.downcast_ref::<&'static str>() {
                                JoinError::Panic(err.to_string())
                            } else {
                                JoinError::Panic("Unknown panic".to_string())
                            }
                        })?
                    ...
                    rx.send(f())
                        .expect("`TaskSpawner` dropped before task competition and `join()` on it")
                });
            }
        }
    }
