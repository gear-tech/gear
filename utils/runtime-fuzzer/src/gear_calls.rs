...
/// The struct for generating gear extrinsics using provided config.
///
/// # Usage
///
...
impl Iterator for GearCalls<'_> {
    ...
    fn next(&mut self) -> Option<Result<GearCall>> {
        loop {
            ...
            let generator = &self.config.generators[self.current_generator].generator;
            let call = generator.generate(&mut self.intermediate_data, &mut self.unstructured);

            self.current_extrinsic += 1;
            if self.current_extrinsic == self.config.generators[self.current_generator].amount {
...
impl Config {
    pub fn new(generators: Vec<RepeatedGenerator>) -> Config {
        Config { generators }
    }
...
/// [`ExtrinsicGenerator`] that should be invoked `amount` times.
pub(crate) struct RepeatedGenerator {
    pub amount: usize,
    pub generator: Box<dyn ExtrinsicGenerator>,
}

impl RepeatedGenerator {
    pub fn new(amount: usize, generator: Box<dyn ExtrinsicGenerator>) -> RepeatedGenerator {
        RepeatedGenerator { amount, generator }
    }
}

/// Trait for the structs that generate parameters to call extrinsics.
