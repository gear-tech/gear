...
use codec::Encode;
use frame_support::{traits::{Currency, GenesisBuild, OnFinalize, OnIdle, OnInitialize}, weights::Weight};
use frame_system as system;
use gear_common::{storage::*, Origin};
use gear_core::message::{StoredDispatch, StoredMessage};
#[cfg(feature = "gear-native")]
use gear_runtime::{
    Authorship, RuntimeEvent, Gear, GearGas, GearMessenger, Runtime, SessionConfig, SessionKeys, System,
};
...
pub fn process_queue(snapshots: &mut Vec<DebugData>, mailbox: &mut Vec<StoredMessage>) {
    while !QueueOf::<Runtime>::is_empty() {
        ...
        for event in System::events() {
            ...
            if let RuntimeEvent::Gear(pallet_gear::Event::UserMessageSent { message, .. }) = &event.event {
...
pub fn run_to_block(n: u32, remaining_weight: Option<u64>, skip_process_queue: bool) {
    while System::block_number() < n {
        ...
        if skip_process_queue {
            ...
            Gear::on_idle(System::block_number(), Weight::from_ref_time(remaining_weight));
        }
    }
}
