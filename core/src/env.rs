...
impl<E: Ext> ExtCarrier<E> {
    ...
    pub fn with<C, T>(&mut self, ctx: C, f: impl FnOnce(C) -> T) -> Result<T, ExtCarrierWithError>
    where
        C: FunctionContext<E>,
    {
        Self::with_fallible(ctx, |ctx| Ok(f(ctx)))
    }

    /// Calls fallible fn with inner ext.
    pub fn with_fallible<C, T, U>(ctx: C, f: impl FnOnce(C) -> Result<T, U>) -> Result<T, U>
    where
        C: FunctionContext<E>,
