...
/// Represents messenger's logic of centralized message processing behaviour.
pub trait Messenger {
    ...
    /// Output error of each storage algorythm.
    ///
    ...
    ///
    /// Present to clarify compiler behaviour over associated types.
    ...
    /// current block by gear's processing algorythm.
    type QueueProcessing: Toggler;

    /// Gear message queue.
    ///
    /// Message queue contains only messages addresed to programs.
    ...
    /// Gear mailbox.
    ///
    /// Mailbox contains only messages addresed to user accounts.
    ...
    /// and send reply on recieved message, if it still present (#642).
