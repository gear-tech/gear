...
/// Enum representing gas tree functions that mutate the state of the existing gas tree.
///
/// Instead of `H256` values, `usize` values are used to represent ids of nodes in the tree.
/// These ids are like **handles** pointing to some existing id in the tree. It's used the next way:
/// ```no_run
...
///     // To understand how `ring_get` works see `RingGet` traits implementation.
...
pub(super) enum GasTreeAction {
    ...
    Split(usize),
    /// Performs split of the node under the bound `usize` index with `u64` amount.
...
/// Generates random vector of `GasTreeAction`s that defines how gas tree is created.
