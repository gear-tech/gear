...
async fn main() {
    ...
    if message == "START" {
        let reply1 =
            msg_async::send_bytes_and_wait_for_reply(unsafe { DEST_0 }, b"PING", GAS_LIMIT, 0)
                .await
                .expect("Error in async message processing");
        let reply2 =
            msg_async::send_bytes_and_wait_for_reply(unsafe { DEST_1 }, b"PING", GAS_LIMIT, 0)
                .await
                .expect("Error in async message processing");
        let reply3 =
            msg_async::send_bytes_and_wait_for_reply(unsafe { DEST_2 }, b"PING", GAS_LIMIT, 0)
                .await
                .expect("Error in async message processing");
