...
main() {
    ...
    mkdir -p utils/runtime-fuzzer/fuzz/check_fuzzer_corpus/main
    dd if=/dev/urandom of=utils/runtime-fuzzer/fuzz/check_fuzzer_corpus/main/check-fuzzer-bytes bs=1 count="$INITIAL_INPUT_SIZE"

    echo " >> Running fuzzer with failpoint"
    RUST_BACKTRACE=1 FAILPOINTS=fail_fuzzer=return ./scripts/gear.sh test fuzzz utils/runtime-fuzzer/fuzz/check_fuzzer_corpus/main wlogs > fuzz_run 2>&1
