...
main() {
    ...
    CONSUME_WITH_LOCK_MUTATION=$(jq 'select(.mutation.fn_name=="consume") | select(.mutation.mutator=="unop_not") | select(.mutation.location_in_file=="608:12-608:13") | .id' target/mutagen/mutations)
