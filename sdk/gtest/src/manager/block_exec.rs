...
impl ExtManager {
    ...
    fn process_messages(&mut self) -> u32 {
        ...
        while self.messages_processing_enabled {
            ...
            gear_processor::handle_journal(journal, self);
    ...
    pub(crate) fn process_dispatch(
        ...
        ProgramsStorageManager::modify_program(destination_id, |program| {
            let Some(program) = program else {
                ...
                return gear_processor::process_code_not_exists(context);
            };
            ...
            match program.as_primary_program() {
                ...
                Program::Exited(inheritor) => {
                    ...
                    return gear_processor::process_program_exited(context, *inheritor);
                }
            };
    ...
    fn process_builtin(&mut self, dispatch: StoredDispatch, gas_limit: u64) -> Vec<JournalNote> {
        ...
        match builtin_call_res {
            Ok(reply_payload) => {
                ...
                gear_processor::process_success(
            ...
            Err(BuiltinActorError::GasAllowanceExceeded) => {
                gear_processor::process_allowance_exceed(incoming_dispatch, destination, 0)
            }
            Err(err) => {
                ...
                gear_processor::process_execution_error(
                    incoming_dispatch,
                    destination,
                    gas_counter.burned(),
    ...
    fn process_program(
        ...
        if !code_metadata.exports().contains(&dispatch_kind) {
            ...
            let notes = gear_processor::process_success(
                SuccessfulDispatchResultKind::Success,
        ...
        gear_processor::process::<Ext<LazyPagesNative>>(
            block_config,
        ...
        .unwrap_or_else(|e| unreachable!("gear-processor logic violated: {}", e))
    }
