...
//! Substrate session manager that, starting from session 2, will select
//! a fraction (determined by a runtime constant) of validators from the initial set
...
pub mod pallet {
    ...
    pub trait Config: pallet_session::Config {
        /// A fraction of initially available validators to be selected for a session
        #[pallet::constant]
        type SelectedFraction: Get<Percent>;
    }
...
impl<T: Config> pallet_session::SessionManager<T::ValidatorId> for Pallet<T> {
    ...
    fn new_session(session_index: sp_staking::SessionIndex) -> Option<Vec<T::ValidatorId>> {
        ...
        // Then for every session we select (deterministically) a certain part of these initial
...
impl<T: Config> Pallet<T> {
    ...
    fn select_validators(
        ...
        let count = sp_std::cmp::max(1, T::SelectedFraction::get() * available_validators_count);
...
mod tests {
    ...
    parameter_types! {
        pub const SelectedFraction: Percent = Percent::from_percent(50); // taking approximately half
    }
    ...
    fn shift_session_manager_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                Pallet::<TestRuntime>::select_validators(0, &all_accs),
                vec![1, 2]
            );

            // at session#1, shift is also 1
            assert_eq!(
                Pallet::<TestRuntime>::select_validators(1, &all_accs),
                vec![2, 3]
            );

            // at session#4, we're wrapping
            assert_eq!(
                Pallet::<TestRuntime>::select_validators(4, &all_accs),
                vec![5, 1]
            );

            // at session#5, we're starting from the beginning again
            assert_eq!(
                Pallet::<TestRuntime>::select_validators(5, &all_accs),
                vec![1, 2]
            );
        });
    }
