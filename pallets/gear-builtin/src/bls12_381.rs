...
fn map_to_g2affine<T: Config>(
    ...
    let to_spend = <T as Config>::WeightInfo::map_to_g2(len).ref_time();
    ...
    let point = match gear_runtime_interface::gear_bls_12_381::map_to_g2(payload) {
        Ok(point) => point,
        Err(e) => {
            log::debug!(
                target: LOG_TARGET,
    ...
    (Ok(Response::MapToG2Affine(point)), gas_spent)
}