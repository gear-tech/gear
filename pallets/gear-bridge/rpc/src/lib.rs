...
pub use pallet_gear_bridge_rpc_runtime_api::GearBridgeApi as GearBridgeRuntimeApi;

#[rpc(server)]
pub trait GearBridgeApi<BlockHash> {
    #[method(name = "gearBridge_merkleProof")]
...
pub struct GearBridge<C, P> {
    /// Shared reference to the client.
    client: Arc<C>,
    _marker: std::marker::PhantomData<P>,
}

impl<C, P> GearBridge<C, P> {
...
impl<C, B: Block> GearBridgeApiServer<B::Hash> for GearBridge<C, B>
where
    C: 'static + ProvideRuntimeApi<B> + HeaderBackend<B>,
    C::Api: GearBridgeRuntimeApi<B>,
{
            ...
            .and_then(|opt| {
                opt.ok_or_else(|| {
                    ErrorObject::owned(
                        8000,
                        "Runtime error",
                        Some(String::from("Element wasn't found in a queue")),
                    )
                })
            })
