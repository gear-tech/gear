...
fn allowed_entry_funcs_works() {
    ...
    let forbidden_func = r#"
        (module
            (import "env" "memory" (memory 1))
            (import "env" "gr_block_height" (func $gr_block_height (result i32)))
            (export "init" (func $init))
            (export "handle_signal" (func $handle_signal))
            (func $init)
            (func $handle_signal
                call $gr_block_height
                drop
            )
        )"#;
    ...
    new_test_ext().execute_with(|| {
        let mut next_block = 2;
        let tests = [
            (allowed_func, DispatchStatus::Success),
            (forbidden_func, DispatchStatus::Failed),
        ];
        for (func, status) in tests {
            let func = ProgramCodeKind::Custom(func).to_bytes();
            ...
            GearPallet::<Test>::deposit_event(Event::UserMessageSent {
                message: trap_reply.message().clone(),
                expiration: None,
            });
            ...
            assert_eq!(dispatch_status(trap_reply.id()), Some(status));
        }
    });
}
