...
use gear_core::{
    ...
    memory::{WasmPage, PageNumber, PageU32Size, PageBuf, PageBufInner},
    message::Message,
    ...
    T::AccountId: Origin,
{
    ...
    pub fn lazy_pages_write_after_read(wasm_pages: WasmPageNumber) -> Result<Exec<T>, &'static str> {
    ...
    pub fn lazy_pages_read_storage_data(wasm_pages: WasmPageNumber) -> Result<Exec<T>, &'static str> {
        let exec = Self::lazy_pages_read(wasm_pages)?;
        let program_id = exec.context.program().id();
        for page in wasm_pages.iter_from_zero().flat_map(|p| p.to_pages_iter::<PageNumber>()) {
