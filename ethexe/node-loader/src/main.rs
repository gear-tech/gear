...
async fn load_node(params: LoadParams) -> Result<()> {
    ...
    if params.workers > PREFUNDED_ACCOUNTS.len() {
        return Err(anyhow::anyhow!(
            "workers must not exceed {} (available prefunded accounts)",
    ...
    for i in 0..params.workers as u32 {
        let account_index = (i + 1) as usize;
        let private_key = PREFUNDED_ACCOUNTS[account_index].private_key;
    ...
    while let Some(result) = init_tasks.join_next().await {
        let (i, address, api) = result??;
        info!(
            "worker {i}: {}",
            PREFUNDED_ACCOUNTS[(i + 1) as usize].address
        );
...
pub static PREFUNDED_ACCOUNTS: &[PrefundedAccount] = &[
    PrefundedAccount {
