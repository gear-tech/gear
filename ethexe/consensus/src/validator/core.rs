...
impl ValidatorCore {
    ...
    pub async fn aggregate_validators_commitment(
        ...
        if last_committed_era_index == block_era + 1 {
            ...
            return Ok(None);
        } else if last_committed_era_index > block_era + 1 {
        ...
        } else if last_committed_era_index < block_era {
            tracing::warn!(
                current_era = %block_era,
                last_committed_era_index = %last_committed_era_index,
        ...
        let election_block = loop {
            ...
            let Some(parent_guard) = self.db.synced_block_read(parent_hash) else {
            ...
            let header = parent_guard.into_simple()?.header;
...
mod tests {
    ...
    async fn test_aggregate_validators_commitment() {
        ...
        ctx.core
            .db
            .block_meta(chain.blocks[7].hash)
            .last_committed_era_index = Some(1);
        let commitment = ctx
            .core
            .aggregate_validators_commitment(&chain.blocks[7].to_simple())
            .await
        ...
        ctx.core
            .db
            .block_meta(chain.blocks[15].hash)
            .last_committed_era_index = Some(0);
        let commitment = ctx
            .core
        ...
        ctx.core
            .db
            .block_meta(chain.blocks[15].hash)
            .last_committed_era_index = Some(3);
        ctx.core
            .aggregate_validators_commitment(&chain.blocks[15].to_simple())
            .await
            .unwrap_err();
