...
trait StateHandler:
    Sized + Into<ValidatorState> + fmt::Display + fmt::Debug + Any + Unpin + Send + 'static
{
    fn context(&self) -> &ValidatorContext;
    ...
    fn process_block_from_producer(
        ...
        DefaultProcessing::block_from_producer(self.into(), block)
    }
    ...
    fn process_validation_reply(
        ...
        DefaultProcessing::validation_reply(self.into(), reply)
    }

    fn poll(self, _cx: &mut Context<'_>) -> Result<ValidatorState> {
        Ok(self.into())
    }
...
impl StateHandler for ValidatorState {
    ...
    fn poll(self, cx: &mut Context<'_>) -> Result<ValidatorState> {
...
impl DefaultProcessing {
    fn new_head(s: ValidatorState, block: SimpleBlockData) -> Result<ValidatorState> {
        Initial::create_with_chain_head(s.into_context(), block)
    }

    fn synced_block(mut s: ValidatorState, data: BlockSyncedData) -> Result<ValidatorState> {
        s.warning(format!("unexpected synced block: {}", data.block_hash));

        Ok(s)
    }

    fn computed_block(mut s: ValidatorState, computed_block: H256) -> Result<ValidatorState> {
