...
async fn ping() {
    ...
    let res = env
        .send_message(res.program_id, b"PING", 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
    ...
    let res = env
        .send_message(ping_id, b"PING", 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
    ...
    let res = env
        .send_message(ping_id, b"PUNK", 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
...
async fn uninitialized_program() {
    ...
    // Case #1: Init failed due to panic in init (decoding).
    {
        ...
        let reply = env
            .send_message(res.program_id, &[], 0, false)
            .await
            .unwrap()
            .wait_for()
            .await
    ...
    // Case #2: async init, replies are acceptable.
    {
        ...
        let init_res = env
            ...
            .unwrap();
        let init_reply = env
            .send_message(init_res.program_id, &init_payload, 0, false)
            .await
            .unwrap();
        ...
        let res = env
            .send_message(init_res.program_id, &[], 0, false)
            .await
            .unwrap()
            .wait_for()
            .await
...
async fn mailbox() {
    ...
    let init_res = env
        .send_message(res.program_id, &env.sender_id.encode(), 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
    ...
    let res = env
        .send_message(pid, &demo_async::Command::Mutex.encode(), 0, false)
        .await
        .unwrap();
...
async fn value_reply_program_to_user() {
    ...
    let _ = env
        .send_message(res.program_id, b"", 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
    ...
    let res = env
        .send_message(piggy_bank_id, b"smash_with_reply", 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
...
async fn value_send_program_to_user_and_claimed() {
    ...
    let res = env
        .send_message(piggy_bank_id, b"smash", 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
...
async fn incoming_transfers() {
    ...
    let res = env
        .send_message(ping_id, b"PING", VALUE_SENT, false)
        .await
        .unwrap()
        .wait_for()
        .await
...
async fn ping_reorg() {
    ...
    let create_program = env
        ...
        .unwrap();
    let init = env
        .send_message(create_program.program_id, b"PING", 0, false)
        .await
        .unwrap();
    ...
    let send_message = env.send_message(ping_id, b"PING", 0, false).await.unwrap();
...
async fn multiple_validators() {
    ...
    let res = env
        ...
        .unwrap();
    let init_res = env
        .send_message(res.program_id, ping_id.encode().as_slice(), 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
    ...
    let res = env
        .send_message(
            async_id,
            demo_async::Command::Common.encode().as_slice(),
            0,
            false,
        )
        .await
...
async fn fast_sync() {
    ...
    for (i, program_id) in program_ids.iter_mut().enumerate() {
        ...
        *program_id = program_info.program_id;
        ...
        let _reply_info = env
            .send_message(program_info.program_id, &value.encode(), 0, false)
            .await
            .unwrap()
            .wait_for()
            .await
    ...
    for (i, program_id) in program_ids.into_iter().enumerate() {
        let reply_info = env
            .send_message(program_id, &(i as u64).encode(), 0, false)
            .await
            .unwrap()
            .wait_for()
            .await
    ...
    for (i, program_id) in program_ids.into_iter().enumerate() {
        let i = (i * 3) as u64;
        let reply_info = env
            .send_message(program_id, &i.encode(), 0, false)
            .await
            .unwrap()
            .wait_for()
            .await
...
async fn value_send_program_to_program() {
    ...
    let res = env
        .send_message(value_sender_id, &(0_u64, VALUE_SENT).encode(), 0, false)
        .await
        .unwrap()
        .wait_for()
        .await
