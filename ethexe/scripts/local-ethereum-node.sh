...
(cd ethexe/contracts && forge clean && forge script script/Router.s.sol:RouterScript --rpc-url "ws://localhost:8545" --broadcast)

BROADCAST_PATH="ethexe/contracts/broadcast/Router.s.sol/31337/run-latest.json"
...
if [[ -e .ethexe.toml ]]; then
  SED_FROM="ethereum_router_address = \"[a-zA-Z0-9]*\""
  SED_TO="ethereum_router_address = \"${PROXY_ADDRESS}\""
  sed -i.bak "s/$SED_FROM/$SED_TO/g" .ethexe.toml
  rm .ethexe.toml.bak
  
  echo "Router address has been updated in .ethexe.toml"
fi
