            ...
            let proposal = match state.get_previously_built_value(height, round).await? {
                ...
                None => {
                    ...
                    if let Some(eth_block_hash) = advance {
                        // One `AdvanceTillEthereumBlock` covers the
                        // whole range from the parent MB's
                        // `last_advanced_block` (exclusive) to
                        // `eth_block_hash` (inclusive) — the executor
                        // walks the canonical chain on its side.
