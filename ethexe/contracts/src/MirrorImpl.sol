...
contract MirrorImpl is IMirror {
    ...
    function performStateTransition(
        Gear.StateTransition calldata _transition
    ) external returns (bytes32) {
        /// @dev Verify that the transition belongs to this contract.
        require(
            _transition.actorId == address(this),
            "actorId must be this contract"
        );
    ...
    function _sendMessages(
        Gear.Message[] calldata _messages
        ...
        for (uint256 i = 0; i < _messages.length; i++) {
            ...
            messagesHashes = bytes.concat(
                messagesHashes,
                Gear.messageHash(message)
            );
    ...
    function _claimValues(
        Gear.ValueClaim[] calldata _claims
    ) private returns (bytes32) {
