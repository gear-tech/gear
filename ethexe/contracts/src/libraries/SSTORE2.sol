...
/**
 * @dev Library for storing byte arrays of data on Ethereum via `CREATE2` opcode.
 *      We use it to store `frost_core::keys::VerifiableSecretSharingCommitment`
 *      structure on-chain and to allow validators to reconstruct `frost_core::keys::PublicKeyPackage`
 *      from publicly accessible on-chain data.
