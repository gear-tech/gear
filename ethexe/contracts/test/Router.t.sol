...
contract RouterTest is Base {
    ...
    function test_validatorsCommitment() public {
        ...
        rollBlocksUntilTimestamp(
            uint256(router.genesisTimestamp() + eraDuration - electionDuration) - 2 * blockDuration
        );
        commitValidators(commitment, true);

        rollBlocksUntilTimestamp(uint256(router.genesisTimestamp() + eraDuration - electionDuration));
        ...
        rollBlocksUntilTimestamp(uint256(router.genesisTimestamp() + eraDuration));
        ...
        rollBlocksUntilTimestamp(uint256(router.genesisTimestamp() + 2 * eraDuration - electionDuration));
        commitValidators(commitment2, false);

        rollBlocksUntilTimestamp(uint256(router.genesisTimestamp() + 2 * eraDuration));
        assertEq(router.validators(), _validators);

        // Do not commit validators - should continue to work with validators from previous era then.
        rollBlocksUntilTimestamp(uint256(router.genesisTimestamp() + 10 * eraDuration));
        assertEq(router.validators(), _validators);

        // Try to commit from wrong validators
        rollBlocksUntilTimestamp(uint256(router.genesisTimestamp() + 10 * eraDuration - electionDuration));
