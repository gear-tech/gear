...
//! [ethexe_common::injected::SignedTxReceipt], it creates the
...
//!
//! [promise_manager::PromiseSubscriptionManager] provides two methods for receiving receipts:
//! - [promise_manager::PromiseSubscriptionManager::on_tx_receipt] receives the receipt signature
//!   from the producer. If it matches a promise already stored in the database, it is sent to the
//!   subscriber.
//! - [promise_manager::PromiseSubscriptionManager::on_computed_promise] receives the promise
//!   body. When RPC receives the corresponding receipt signature, it sends the signed receipt to the
//!   subscriber.
