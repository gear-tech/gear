...
use cumulus_client_service::{
    build_relay_chain_interface, prepare_node_config, start_collator, start_full_node, StartCollatorParams, StartFullNodeParams,
};
...
// #[cfg(feature = "rococo-gear-native")]
pub struct RococoGearRuntimeExecutor;

// #[cfg(feature = "rococo-gear-native")]
impl sc_executor::NativeExecutionDispatch for RococoGearRuntimeExecutor {
    type ExtendHostFunctions = ExtendHostFunctions;

    fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
        rococo_gear_runtime::api::dispatch(method, data)
    }

    fn native_version() -> sc_executor::NativeVersion {
        rococo_gear_runtime::native_version()
    }
}

// #[cfg(feature = "rococo-gear-native")]
type ParachainNativeExecutor = RococoGearRuntimeExecutor;

type ParachainExecutor = sc_executor::NativeElseWasmExecutor<ParachainNativeExecutor>;
...
type ParachainBlockImport = cumulus_client_consensus_common::ParachainBlockImport<Block, Arc<ParachainClient>, ParachainBackend>;
...
> {
	...
	let import_queue = rococo_gear_build_import_queue(
		client.clone(),
		block_import.clone(),
		config,
...
pub fn rococo_gear_build_import_queue(
...
pub async fn start_rococo_gear_node(
    parachain_config: Configuration,
    polkadot_config: Configuration,
    collator_options: CollatorOptions,
    para_id: ParaId,
