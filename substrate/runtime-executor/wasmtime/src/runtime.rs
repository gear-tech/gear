...
use crate::{
    ...
    util as memory_util,
    util::replace_strategy_if_broken,
};
...
fn common_config(semantics: &Semantics) -> std::result::Result<wasmtime::Config, WasmError> {
    ...
    config.memory_guaranteed_dense_image_size(match semantics.heap_alloc_strategy {
        HeapAllocStrategy::Dynamic { maximum_pages } => maximum_pages
            .map(|p| p as u64 * WASM_PAGE_SIZE)
            .unwrap_or(u64::MAX),
...
where
    H: HostFunctions,
{
    // SAFETY: This function carries the same caller contract as `do_create_runtime`.
    unsafe { do_create_runtime::<H>(CodeSupplyMode::Precompiled(compiled_artifact_path), config) }
}
    ...
    H: HostFunctions,
{
    ...
    if let Some(ref cache_path) = config.cache_path
        && let Err(reason) = setup_wasmtime_caching(cache_path, &mut wasmtime_config)
    {
        log::warn!(
    ...
    let (module, instantiation_strategy) = match code_supply_mode {
        ...
        CodeSupplyMode::Precompiled(compiled_artifact_path) => {
            ...
            let module =
                unsafe { wasmtime::Module::deserialize_file(&engine, compiled_artifact_path) }
        ...
        CodeSupplyMode::PrecompiledBytes(compiled_artifact_bytes) => {
            ...
            let module = unsafe { wasmtime::Module::deserialize(&engine, compiled_artifact_bytes) }
                .map_err(|e| WasmError::Other(format!("cannot deserialize module: {:#}", e)))?;
...
fn perform_call(
    ...
    let host_state = instance_wrapper
        .store_mut()
        .data_mut()
        .host_state
        .take()
...
fn extract_output_data(
    ...
    if memory_util::checked_range(output_ptr as usize, output_len as usize, memory_size).is_none() {
        Err(WasmError::Other(
            "output exceeds bounds of wasm memory".into(),
        ))?
    }
