...
use crate::utils::{
    ...
    create_function_ident_with_version, create_host_function_ident, generate_crate_access,
    get_function_argument_names, get_function_argument_names_and_types,
    get_function_argument_types, get_function_arguments, get_runtime_interface,
};

use syn::{
    Error, Ident, ItemTrait, Pat, Result, ReturnType, Signature, TraitItemFn, spanned::Spanned,
};
...
fn generate_extern_host_function(
    ...
    for (nth, arg) in get_function_arguments(&method.sig).enumerate() {
        ...
        let ffi_name = Ident::new(
            &format!("__runtime_interface_ffi_value_{}_", nth),
            arg.pat.span(),
        );
        let destructor_name = Ident::new(
            &format!("__runtime_interface_arg_destructor_{}_", nth),
            arg.pat.span(),
        );
...
fn generate_host_function_implementation(
    ...
    for (host_name, host_ty) in get_function_argument_names_and_types(&method.sig) {
        ...
        let host_name_ident = match *host_name {
            ...
            _ => unreachable!(
                "`generate_ffi_value_var_name` above would return an error on `Pat` != `Ident`; qed"
            ),
        };

        let ffi_ty = quote! { <#host_ty as #crate_::RIType>::FFIType };
        ffi_args_prototype.push(quote! { #ffi_name: #ffi_ty });
        ffi_arg_types.push(ffi_ty.clone());
    ...
    let register_body = quote! {
        ...
        registry.register_static::<
            (
                #crate_::sp_wasm_interface::wasmtime::Caller<'_, T::State>,
                #(#ffi_arg_types,)*
            ),
            #crate_::sp_wasm_interface::wasmtime::Result<#ffi_return_ty>,
        >(
