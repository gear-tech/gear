...
use crate::utils::{
    RuntimeInterface, RuntimeInterfaceFunction, create_exchangeable_host_function_ident,
    ...
    get_function_argument_types_without_ref, get_function_arguments, get_runtime_interface,
};
...
pub fn generate(trait_def: &ItemTrait, is_wasm_only: bool) -> Result<TokenStream> {
    ...
    let extern_host_function_impls = interface.latest_versions_to_call().try_fold(
        TokenStream::new(),
    ...
    let host_functions_struct =
        generate_host_functions_struct(trait_def, &interface, is_wasm_only)?;
...
fn generate_host_functions_struct(
    ...
    for (version, method) in interface.all_versions() {
...
fn generate_host_function_implementation(
        ...
        get_function_argument_names_and_types_without_ref(&method.sig).zip(ref_and_mut)
    {
        ...
        if ref_and_mut
            .map(|(_, token_mut)| token_mut.is_some())
            .unwrap_or(false)
        {
