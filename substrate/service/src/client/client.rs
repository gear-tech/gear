    ...
    Block::Header: Clone,
{
        ...
        B: 'static,
    {
        ...
        spawn_handle.spawn(
            "notification-pinning-worker",
            None,
            Box::pin(unpin_worker.run()),
        );
        ...
        <Self as ProvideRuntimeApi<Block>>::Api: CoreApi<Block> + ApiExt<Block>,
    {
        ...
        let gap_block = info
            .block_gap
            .is_some_and(|(start, _)| *import_headers.post().number() == start);
            ...
            Some(storage_changes) => {
                let storage_changes = match storage_changes {
                    sc_consensus::StorageChanges::Changes(storage_changes) => {
                        self.backend
                            .begin_state_operation(&mut operation.op, parent_hash)?;
    ...
    fn apply_finality_with_block_hash(
        ...
        if let Some(retracted) = route_from_finalized.retracted().first() {
            warn!(
    ...
    pub fn block_status(&self, hash: Block::Hash) -> sp_blockchain::Result<BlockStatus> {
        ...
        if self.importing_block.read().as_ref() == Some(&hash) {
    ...
    Block: BlockT,
{
    ...
    fn storage_collection(
        ...
        let mut current_child = if start_key.len() == 2 {
            let start_key = start_key.first().expect("checked len");
        ...
        let mut current_key = start_key.last().cloned().unwrap_or_default();
        ...
        loop {
            ...
            if let Some((child, child_root)) = switch_child_key.take() {
                result[0].0.key_values.extend(entries);
