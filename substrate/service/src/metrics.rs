    ...
    fn update<T: Block>(
        ...
        if let Some(metrics) = self.metrics.as_ref() {
            ...
            metrics
                ...
                .set(txpool_status.ready as u64);

            if let Some(info) = info.usage.as_ref() {
                metrics
                    .database_cache
                    .set(info.memory.database_cache.as_bytes() as u64);
                metrics
                    .state_cache
                    .set(info.memory.state_cache.as_bytes() as u64);
            }
        }
        ...
        if let Some(net_status) = net_status {
            ...
            let (avg_bytes_per_sec_inbound, avg_bytes_per_sec_outbound) = if elapsed > 0 {
                ...
                (
                    diff_bytes_inbound
                        .checked_div(elapsed)
                        .unwrap_or(diff_bytes_inbound),
                    diff_bytes_outbound
                        .checked_div(elapsed)
                        .unwrap_or(diff_bytes_outbound),
                )
            } else {
