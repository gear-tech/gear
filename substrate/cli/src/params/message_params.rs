...
mod tests {
    ...
    fn message_decode_immediate() {
        for (name, input, hex, output) in test_closures() {
            ...
            match output {
                Some(output) => {
                    let message =
                        message.unwrap_or_else(|_| panic!("{}: should decode but did not", name));
                    assert_eq!(message, output, "{}: decoded a wrong message", name);
                }
                None => {
                    message
                        .err()
                        .unwrap_or_else(|| panic!("{}: should not decode but did", name));
                }
            }
        }
    }
    ...
    fn test_closures() -> Vec<(&'static str, &'static str, bool, Option<&'static [u8]>)> {
        vec![
            ...
            (
                "decode_no_hex_with_hex_string_works",
                "0xffffffff",
                false,
                Some(b"0xffffffff"),
            ),
            ...
            (
                "decode_hex_without_prefix_works",
                "00112233",
                true,
                Some(&[0, 17, 34, 51]),
            ),
            (
                "decode_hex_uppercase_works",
                "0xaAbbCCDd",
                true,
                Some(&[170, 187, 204, 221]),
            ),
