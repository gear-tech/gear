...
mod tests {
    ...
    fn test_node_key_config_input() {
        fn secret_input(net_config_dir: &PathBuf) -> error::Result<()> {
            NodeKeyType::value_variants().iter().try_for_each(|t| {
                ...
                params
                    .node_key(net_config_dir, Role::Authority, false)
                    .and_then(|c| match c {
    ...
    fn test_node_key_config_default() {
        ...
        fn some_config_dir(
            ...
            with_def_params(
                |params| {
                    let dir = net_config_dir.clone();
                    let key_type = params.node_key_type;
                    params
                        .node_key(net_config_dir, role, is_dev)
                        .and_then(move |c| match c {
                            NodeKeyConfig::Ed25519(sc_network::config::Secret::File(ref f))
                                if key_type == NodeKeyType::Ed25519
        ...
        assert!(
            some_config_dir(&PathBuf::from_str("x").unwrap(), false, Role::Full, false).is_ok()
        );
        assert!(some_config_dir(
