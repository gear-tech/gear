    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for input in s.split(',') {
            let (key, val) = input
                .trim()
                .split_once('=')
                .ok_or_else(|| invalid_input(input))?;
            let key = key.trim();
            let val = val.trim();
            ...
            match key {
                RPC_LISTEN_ADDR => {
                    ...
                    let val: SocketAddr = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_LISTEN_ADDR, val))?;
                ...
                RPC_MAX_REQUEST_SIZE => {
                    ...
                    let val = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_MAX_RESPONSE_SIZE, val))?;
                ...
                RPC_MAX_SUBS_PER_CONN => {
                    ...
                    let val = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_MAX_SUBS_PER_CONN, val))?;
                ...
                RPC_MAX_BUF_CAP_PER_CONN => {
                    ...
                    let val = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_MAX_BUF_CAP_PER_CONN, val))?;
                ...
                RPC_RATE_LIMIT => {
                    ...
                    let val = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_RATE_LIMIT, val))?;
                    rate_limit = Some(val);
                }
                ...
                RPC_RETRY_RANDOM_PORT => {
                    ...
                    let val = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_RETRY_RANDOM_PORT, val))?;
                ...
                RPC_DISABLE_BATCH => {
                    ...
                    let val = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_DISABLE_BATCH, val))?;
                ...
                RPC_BATCH_LIMIT => {
                    ...
                    let val = val
                        .parse()
                        .map_err(|_| invalid_value(RPC_BATCH_LIMIT, val))?;
...
impl From<RpcEndpoint> for sc_service::config::RpcEndpoint {
    fn from(val: RpcEndpoint) -> Self {
        sc_service::config::RpcEndpoint {
            batch_config: val.batch_config,
            listen_addr: val.listen_addr,
            max_buffer_capacity_per_connection: val.max_buffer_capacity_per_connection,
            max_connections: val.max_connections,
            max_payload_in_mb: val.max_payload_in_mb,
            max_payload_out_mb: val.max_payload_out_mb,
            max_subscriptions_per_connection: val.max_subscriptions_per_connection,
            rpc_methods: val.rpc_methods.into(),
            rate_limit: val.rate_limit,
            rate_limit_trust_proxy_headers: val.rate_limit_trust_proxy_headers,
            rate_limit_whitelisted_ips: val.rate_limit_whitelisted_ips,
            cors: val.cors,
            retry_random_port: val.retry_random_port,
...
mod tests {
    ...
    fn parse_rpc_endpoint_all() {
        ...
        assert!(endpoint.is_optional);
        assert!(endpoint.retry_random_port);
        ...
        assert!(endpoint.rate_limit_trust_proxy_headers);
        assert_eq!(
