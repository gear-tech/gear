...
impl From<DatabasePruningMode> for PruningMode {
    fn from(val: DatabasePruningMode) -> Self {
        match val {
...
impl From<DatabasePruningMode> for BlocksPruning {
    fn from(val: DatabasePruningMode) -> Self {
        match val {
...
mod tests {
    ...
    fn pruning_params_parse_works() {
        ...
        assert!(matches!(
            pruning.state_pruning,
            Some(DatabasePruningMode::Custom(1000))
        ));
        assert!(matches!(
            pruning.blocks_pruning,
            DatabasePruningMode::Custom(1000)
        ));
        ...
        assert!(matches!(
            dbg!(pruning.state_pruning),
            Some(DatabasePruningMode::ArchiveCanonical)
        ));
        assert!(matches!(
            pruning.blocks_pruning,
