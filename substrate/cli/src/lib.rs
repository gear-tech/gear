...
#![allow(
    ...
    reason = "Preserve the pinned Polkadot SDK CLI API while vendoring the crate"
)]
...
pub trait SubstrateCli: Sized {
    ...
    where
        Self: Parser + Sized,
    {
        <Self as SubstrateCli>::from_iter(std::env::args_os())
    }
        ...
        F: FnOnce(&mut LoggerBuilder, &Configuration),
    {
        ...
        command.init(
            &Self::support_url(),
            &Self::impl_version(),
            |logger_builder| logger_hook(logger_builder, &config),
        )?;
