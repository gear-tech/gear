...
mod impl_ {
    ...
    impl<'a, 'b> sandbox_env::SandboxContext for SandboxContext<'a, 'b> {
        ...
        fn read_memory_into(
            ...
            let range =
                gear_sandbox_native::util::checked_range(address.into(), dest.len(), memory.len())
        ...
        fn write_memory(
            ...
            let range =
                gear_sandbox_native::util::checked_range(address.into(), data.len(), memory.len())
...
pub trait Sandbox {
    ...
    fn instantiate(
        ...
        self.with_caller_mut(context_ptr as *mut (), |context_ptr, caller| {
            ...
            let instance_idx_or_err_code = match result {
                Ok(instance) => {
                    instance.register(context.store.get(store_data_key), dispatch_thunk)
                }
                Err(sandbox_env::InstantiationError::StartTrapped) => {
    ...
    fn memory_set(
        ...
        self.with_caller_mut(context_ptr as *mut (), |context_ptr, caller| {
            ...
            context.result =
                match sandboxed_memory.write_from(Pointer::new(context.offset), &buffer) {
                    Ok(_) => sandbox_env::env::ERR_OK,
                    Err(_) => sandbox_env::env::ERR_OUT_OF_BOUNDS,
                };
        });
